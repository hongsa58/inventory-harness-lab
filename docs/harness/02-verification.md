...
- 보호 경로 변경에는 사람이 명시적으로 지시한 정확한 `SSOT_APPROVED_PATHS` 범위가 필요하다. 승인 대상은 그 지시 범위 안의 경로로만 한정한다.
- 보호 경로를 범위 밖에서 변경하거나 승인 없이 변경하면 `NEEDS_HUMAN`을 출력하고 실패한다. 보호 경로 목록에 있거나 같은 커밋에 포함되어 있다는 사실만으로 범위 밖 변경이 승인되지는 않는다.
- 로컬과 CI는 동일한 검사기를 사용하며, CI는 사람이 직접 남긴 승인 기록을 신뢰된 승인 작업이 확인한 출력만 전달한다. AI·자동화가 생성하거나 대리한 기록, 커밋 메타데이터만으로 추론한 값은 승인 근거가 아니다.
...
보호 경로 변경이 있는 경우 CI의 승인 입력은 사람이 직접 작성해 남긴 승인 기록에서 확인한 정확한 경로 범위여야 한다. AI·자동화가 생성·대리 작성·추론한 내용이나 커밋 작성자·커미터·브랜치·Issue·PR 설명·환경변수만으로는 사람의 승인 기록을 대신할 수 없다. 변경 경로가 사람의 명시적 지시 범위를 벗어나거나 직접 작성된 승인 기록을 확인할 수 없으면 Protected를 실패시키고 `NEEDS_HUMAN`을 출력한다.

현재 검사기는 승인 입력의 형식과 경로 범위를 확인하지만, 커밋 메타데이터만으로 승인 기록이 실제 사람에 의해 직접 작성되었는지 독립적으로 증명하지는 못한다. 따라서 CI에서 직접 작성 여부를 보장하려면 신뢰된 승인 작업이 사람이 남긴 기록의 출처를 별도로 인증·검증해야 하며, 그 검증 결과만 `SSOT_APPROVED_PATHS`로 전달해야 한다.
