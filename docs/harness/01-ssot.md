...
SSOT 변경 필요 발견
  → AI가 변경안·이유·영향 범위 제시
  → 권한 있는 사람이 승인 대상 경로 범위를 직접 명시
  → 사람이 직접 남긴 승인 기록 확인
  → 승인된 범위만 변경
  → 관련 Issue·구현·검증 추적
```

승인 대상은 권한 있는 사람이 작업 지시에서 명시한 정확한 경로 범위로만 한정한다. AI가 발견·추가·추론한 경로, 지시 범위 밖의 경로, 보호 경로 목록에 있다는 사실만으로는 승인 대상이 되지 않는다.

승인 기록은 사람이 직접 작성해 남긴 기록이어야 한다. AI나 자동화가 대신 작성·생성·복사한 기록, 또는 커밋 메타데이터만으로 추론한 기록은 사람의 승인 기록으로 인정하지 않는다.
...
보호 경로는 SSOT와 검증 경로의 권위를 변경할 수 있는 파일이다. 보호 경로를 변경하려면 사람이 명시적으로 지시한 정확한 경로 범위를 외부의 신뢰된 승인 입력으로 제공해야 한다.

승인 대상으로 삼는 범위는 사람이 작업 지시에서 직접 명시한 저장소 상대 경로 집합으로 한정한다. 지시 범위 밖의 보호 경로는 보호 경로 목록에 있거나 같은 작업에 포함되어 보이더라도 승인 대상이 아니며, 별도의 명시적 지시와 승인 없이는 통과할 수 없다.

```text
SSOT_APPROVED_PATHS=docs/06-architecture.md,scripts/check-architecture.ts
```

- 승인 범위는 저장소 상대 경로의 목록이어야 하며, 변경된 보호 경로를 빠짐없이 정확히 포함해야 한다.
- 승인 기록은 권한 있는 사람이 직접 작성해 남긴 기록이어야 한다. AI·자동화가 생성·대리 작성·복사한 기록이나 커밋 메타데이터만으로 추론한 내용은 사람의 승인 기록으로 인정하지 않는다.
- 커밋 작성자·커미터·브랜치·Issue·PR 설명·일반 환경변수·커밋 trailer만으로는 사람의 지시 또는 직접 작성 사실을 증명하지 못한다.
- AI가 임의로 승인 범위를 만들거나 보호 경로를 변경한 경우 승인되지 않는다. 보호 경로의 미커밋 변경도 실패한다.
- 로컬과 CI는 동일한 `npm run check:protected` 검사기를 사용한다. 로컬에서는 사람이 직접 범위를 지시해 `SSOT_APPROVED_PATHS`를 설정할 수 있지만, CI에서는 사람이 직접 남긴 승인 기록을 신뢰된 승인 작업이 검증한 출력만 전달한다.

CI의 승인 입력은 사람이 직접 남긴 승인 기록에서 확인한 정확한 경로 범위여야 한다. AI가 생성한 환경변수나 자동화가 임의로 만든 범위는 사람이 직접 남긴 기록을 대체하지 못한다.

- CI는 `SSOT_VERIFY_BASE`에 PR 대상 기준 커밋을 전달하고, 로컬은 기본적으로 `origin/main`(없으면 `HEAD^`)과 현재 HEAD를 비교한다.
- 검사 결과가 승인되지 않으면 `NEEDS_HUMAN` 상태를 선언하고, 사람의 승인 전에는 영향을 받는 검증을 진행하지 않는다.
- 보호 경로 목록 자체의 변경도 동일하게 검사하며, 검사기의 우회·bootstrap 예외는 없다.
...
사람이 승인한 변경은 정확한 범위를 지정해 검증한다. 기존 `SSOT-Approved-By` trailer는 감사용 텍스트일 뿐 승인 근거가 아니다.

```bash
SSOT_APPROVED_PATHS=docs/harness/01-ssot.md npm run check:protected
```

CI에서는 신뢰된 승인 작업이 현재 PR head와 보호 경로의 정확한 범위를 확인한 뒤 `SSOT_APPROVED_PATHS`를 설정하고 같은 명령을 실행한다. 직접 push, PR 설명, AI가 생성한 환경변수는 승인으로 인정하지 않는다.
